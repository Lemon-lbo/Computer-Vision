"""
SPAN + ManTraNet evaluation on CASIA1 -- same perturbations as Table 5.

CASIA1 contains:
  Sp/ -- 921 spliced/tampered images (forged, label=1)
  Au/ -- 800 authentic images (label=0)

No pixel-level ground truth masks are provided in CASIA1, so evaluation
is done at IMAGE LEVEL:
  - Image forgery score = max pixel value in the model's prediction map
  - Image-level AUC (all forged vs. all authentic)
  - Image-level F1 at 0.5 threshold

This is the standard evaluation protocol when only image-level labels are
available (as used in many CASIA papers).
"""

import os, sys, glob, time, warnings
warnings.filterwarnings('ignore')

import numpy as np
import cv2
import torch

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from span_pytorch      import build_span
from mantranet_pytorch import build_mantranet

CASIA_DIR    = './CASIA1'
SPAN_WEIGHTS = './IRIS0-SPAN/PixelAttention32.h5'
MT_WEIGHTS   = './ManTraNet_Ptrain4.h5'
DEVICE       = 'cuda' if torch.cuda.is_available() else 'cpu'


# -- perturbations (same as Columbia Table 5) -------------------------

def apply_resize(img, scale):
    h, w = img.shape[:2]
    nh, nw = max(1, int(h * scale)), max(1, int(w * scale))
    return cv2.resize(cv2.resize(img, (nw, nh), cv2.INTER_LINEAR), (w, h), cv2.INTER_LINEAR)

def apply_blur(img, k):
    kk = k if k % 2 == 1 else k + 1
    return cv2.GaussianBlur(img, (kk, kk), 0)

def apply_noise(img, sigma):
    noise = np.random.randn(*img.shape).astype('float32') * sigma
    return np.clip(img.astype('float32') + noise, 0, 255).astype('uint8')

def apply_jpeg(img, quality):
    _, enc = cv2.imencode('.jpg', img, [int(cv2.IMWRITE_JPEG_QUALITY), quality])
    return cv2.imdecode(enc, cv2.IMREAD_COLOR)

PERTURBATIONS = [
    ('No manipulation',               None),
    ('Resize (0.78x)',                lambda x: apply_resize(x, 0.78)),
    ('Resize (0.25x)',                lambda x: apply_resize(x, 0.25)),
    ('No manipulation',               None),
    ('GaussianBlur (kernel size=3)',  lambda x: apply_blur(x, 3)),
    ('GaussianBlur (kernel size=15)', lambda x: apply_blur(x, 15)),
    ('No manipulation',               None),
    ('GaussianNoise (sigma=3)',       lambda x: apply_noise(x, 3)),
    ('GaussianNoise (sigma=15)',      lambda x: apply_noise(x, 15)),
    ('No manipulation',               None),
    ('JPEGCompress (quality=100)',    lambda x: apply_jpeg(x, 100)),
    ('JPEGCompress (quality=50)',     lambda x: apply_jpeg(x, 50)),
]


# -- metrics ----------------------------------------------------------

def _roc_auc(y_true, y_score):
    y_true  = np.asarray(y_true,  'float32').ravel()
    y_score = np.asarray(y_score, 'float32').ravel()
    idx = np.argsort(-y_score)
    yt, ys = y_true[idx], y_score[idx]
    dist = np.concatenate([np.where(np.diff(ys))[0], [len(yt) - 1]])
    tps  = np.concatenate([[0], np.cumsum(yt)[dist]])
    fps  = np.concatenate([[0], dist + 1 - np.cumsum(yt)[dist]])
    fpr  = fps / (fps[-1] + 1e-10)
    tpr  = tps / (tps[-1] + 1e-10)
    _trapz = getattr(np, 'trapezoid', None) or getattr(np, 'trapz')
    return float(_trapz(tpr, fpr))

def _f1(y_true_bin, y_pred_bin):
    tp = np.logical_and( y_true_bin,  y_pred_bin).sum()
    fp = np.logical_and(~y_true_bin,  y_pred_bin).sum()
    fn = np.logical_and( y_true_bin, ~y_pred_bin).sum()
    d  = 2 * tp + fp + fn
    return float(2 * tp / d) if d > 0 else 0.0

def compute_image_metrics(labels, scores):
    """
    labels : list of 0/1 (0=authentic, 1=forged)
    scores : list of float (image forgery score, higher = more likely forged)
    Returns image-level AUC (%) and F1 (%) at 0.5 threshold.
    """
    labels = np.array(labels, 'float32')
    scores = np.array(scores, 'float32')
    auc = _roc_auc(labels, scores) * 100
    pred_bin = scores > 0.5
    f1  = _f1(labels.astype(bool), pred_bin) * 100
    return auc, f1


# -- dataset ----------------------------------------------------------

def load_casia(casia_dir):
    """Load all Sp (forged, label=1) and Au (authentic, label=0) images."""
    sp_dir = os.path.join(casia_dir, 'Sp')
    au_dir = os.path.join(casia_dir, 'Au')
    data = []
    for p in sorted(glob.glob(os.path.join(sp_dir, '*.jpg'))):
        img = cv2.imread(p, cv2.IMREAD_COLOR)
        if img is None:
            continue
        rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        data.append((rgb, 1))      # label 1 = forged
    for p in sorted(glob.glob(os.path.join(au_dir, '*.jpg'))):
        img = cv2.imread(p, cv2.IMREAD_COLOR)
        if img is None:
            continue
        rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        data.append((rgb, 0))      # label 0 = authentic
    return data


# -- inference helpers ------------------------------------------------

@torch.no_grad()
def infer_span(model, rgb_uint8):
    x = torch.from_numpy(
            rgb_uint8.astype('float32') / 255.0 * 2.0 - 1.0
        ).permute(2, 0, 1).unsqueeze(0).to(DEVICE)
    with torch.amp.autocast(device_type='cuda'):
        y = model(x)[0, 0]
    return float(y.float().cpu().numpy().max())   # image-level score = max pixel


@torch.no_grad()
def infer_mt(model, rgb_uint8):
    x = torch.from_numpy(
            rgb_uint8.astype('float32') / 255.0 * 2.0 - 1.0
        ).permute(2, 0, 1).unsqueeze(0).to(DEVICE)
    with torch.amp.autocast(device_type='cuda'):
        y = model(x)[0, 0]
    return float(y.float().cpu().numpy().max())   # image-level score = max pixel


# -- evaluation loop --------------------------------------------------

def run_one(span, mt, data, pert_fn, label):
    labels, span_scores, mt_scores = [], [], []
    n = len(data)
    for done, (rgb, lbl) in enumerate(data, 1):
        img = pert_fn(rgb) if pert_fn else rgb
        span_scores.append(infer_span(span, img))
        mt_scores.append(infer_mt(mt, img))
        labels.append(lbl)
        if done % 100 == 0:
            torch.cuda.empty_cache()
        print(f'\r  {label}: {done}/{n}', end='', flush=True)
    print()
    auc_s, f1_s = compute_image_metrics(labels, span_scores)
    auc_m, f1_m = compute_image_metrics(labels, mt_scores)
    return auc_s, f1_s, auc_m, f1_m


# -- main -------------------------------------------------------------

def main():
    print(f'Device: {DEVICE}')
    if DEVICE == 'cuda':
        print(f'GPU: {torch.cuda.get_device_name(0)}')

    print('Loading SPAN ...')
    span = build_span(SPAN_WEIGHTS, device=DEVICE)

    print('Loading ManTraNet ...')
    mt = build_mantranet(MT_WEIGHTS, device=DEVICE)

    print('Loading CASIA1 dataset into RAM ...')
    data = load_casia(CASIA_DIR)
    n_forged    = sum(1 for _, l in data if l == 1)
    n_authentic = sum(1 for _, l in data if l == 0)
    print(f'Loaded {n_forged} forged + {n_authentic} authentic = {len(data)} images.')
    print('Evaluation metric: IMAGE-LEVEL AUC + F1 (no pixel masks in CASIA1).\n')

    # Warmup: run through a small sample
    print('Warming up GPU ...', end='', flush=True)
    with torch.no_grad(), torch.amp.autocast(device_type='cuda'):
        for rgb, _ in data[:5]:
            x = torch.from_numpy(rgb.astype('float32')/255.0*2.0-1.0).permute(2,0,1).unsqueeze(0).to(DEVICE)
            span(x); mt(x)
    torch.cuda.synchronize()
    print(' done.\n')

    W = 42
    sep = '-' * (W + 36)
    hdr = f"{'Perturbation':<{W}} {'SPAN AUC':>9} {'MT AUC':>9} {'SPAN F1':>8} {'MT F1':>8}"
    print(sep); print(hdr); print(sep)

    results       = []
    cache_nomanip = None

    for name, fn in PERTURBATIONS:
        if name == 'No manipulation' and cache_nomanip is not None:
            auc_s, f1_s, auc_m, f1_m = cache_nomanip
        else:
            t0 = time.time()
            auc_s, f1_s, auc_m, f1_m = run_one(span, mt, data, fn, name)
            elapsed = time.time() - t0
            print(f'  -> done in {elapsed:.0f}s')
            if name == 'No manipulation':
                cache_nomanip = (auc_s, f1_s, auc_m, f1_m)

        print(f'{name:<{W}} {auc_s:>9.2f} {auc_m:>9.2f} {f1_s:>8.2f} {f1_m:>8.2f}')
        results.append((name, auc_s, f1_s, auc_m, f1_m))

    print(sep)

    csv_path = 'casia_results.csv'
    with open(csv_path, 'w') as f:
        f.write('Perturbation,SPAN_Image_AUC,SPAN_F1,MT_Image_AUC,MT_F1\n')
        for r in results:
            f.write(f'{r[0]},{r[1]:.4f},{r[2]:.4f},{r[3]:.4f},{r[4]:.4f}\n')
    print(f'\nSaved -> {csv_path}')


if __name__ == '__main__':
    main()
